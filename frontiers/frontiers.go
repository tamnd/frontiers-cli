// Package frontiers is the library behind the frontiers command line:
// the HTTP client, request shaping, and the typed data models for Frontiers
...
const DefaultUserAgent = "Mozilla/5.0 (compatible; frontiers-cli/0.1; +https://github.com/tamnd/frontiers-cli)"
...
// Client talks to the CrossRef API for Frontiers papers.
type Client struct {
	cfg        Config
...
// Recent fetches recently published Frontiers papers.
func (c *Client) Recent(ctx context.Context, limit int) ([]Paper, error) {
	if limit <= 0 {
		limit = 20
	}
	apiURL := fmt.Sprintf("%s/members/1965/works?rows=%d&sort=published&order=desc", c.cfg.BaseURL, limit)
	return c.fetch(ctx, apiURL, limit)
}

// Search searches Frontiers papers by keyword.
func (c *Client) Search(ctx context.Context, query string, limit int) ([]Paper, error) {
	if limit <= 0 {
		limit = 20
	}
	apiURL := fmt.Sprintf(
		"%s/members/1965/works?rows=%d&sort=published&order=desc&query=%s",
