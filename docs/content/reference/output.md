...
describes what users get. Pick a format with `-o`, or let frontiers choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
frontiers <command> -o table   # aligned columns for reading
frontiers <command> -o jsonl   # one JSON object per line, for piping
frontiers <command> -o json    # a single JSON array
frontiers <command> -o csv     # spreadsheet friendly
frontiers <command> -o tsv     # tab-separated
frontiers <command> -o url     # just the URL column
frontiers <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
frontiers <command> --fields id,title,url
```
...
```bash
frontiers <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
frontiers <command>            # a table, because this is a terminal
frontiers <command> | wc -l    # JSONL, because this is a pipe
```
